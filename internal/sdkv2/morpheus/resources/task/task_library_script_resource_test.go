...
func TestAccMorpheusTaskLibraryScriptExampleOk(t *testing.T) {
	t.Parallel()
	...
	resourceConfig, err := task.RenderTaskLibraryScriptConfig(t, map[string]string{
		"Name": name,
	})
	if err != nil {
		t.Fatal(err)
	...
	checks := []resource.TestCheckFunc{
		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_library_script.example",
			"allow_custom_config",
			"true",
		),
		...
		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_library_script.example",
			"code",
			"tf-example-library-script-task",
		),
