// (C) Copyright 2026 Hewlett Packard Enterprise Development LP

package task_test

import (
	"testing"

	"github.com/hashicorp/terraform-plugin-testing/helper/acctest"
	"github.com/hashicorp/terraform-plugin-testing/helper/resource"

	"github.com/HPE/terraform-provider-hpe/internal/framework/subproviders/morpheus"
	"github.com/HPE/terraform-provider-hpe/internal/framework/subproviders/morpheus/testhelpers"
	sdkv2morpheus "github.com/HPE/terraform-provider-hpe/internal/sdkv2/morpheus"
	"github.com/HPE/terraform-provider-hpe/internal/sdkv2/morpheus/resources/task"
)

func TestAccMorpheusTaskAnsiblePlaybookExampleOk(t *testing.T) {
	t.Parallel()

	defer testhelpers.RecordResult(t)

	if testing.Short() {
		t.Skip("Skipping slow test in short mode")
	}

	// t.Skip("Skipping due to API error")
	// t.Skip("Skipping due to missing infrastructure in test environment")
	// t.Skip("Skipping due to missing resource implementation")
	// t.Skip("Skipping due to mismatch between Morpheus API and Terraform schema")

	providerConfig := testhelpers.ProviderBlock()

	name := acctest.RandomWithPrefix(t.Name())

	resourceConfig, err := task.RenderTaskAnsiblePlaybookConfig(t, map[string]string{
		"Name": name,
	})
	if err != nil {
		t.Fatal(err)
	}

	checks := []resource.TestCheckFunc{
		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_ansible_playbook.example",
			"allow_custom_config",
			"true",
		),

		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_ansible_playbook.example",
			"ansible_repo_id",
			"5",
		),

		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_ansible_playbook.example",
			"code",
			"tfansibletest",
		),

		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_ansible_playbook.example",
			"command_options",
			"-b",
		),

		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_ansible_playbook.example",
			"execute_target",
			"local",
		),

		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_ansible_playbook.example",
			"git_ref",
			"master",
		),

		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_ansible_playbook.example",
			"labels.#",
			"2",
		),

		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_ansible_playbook.example",
			"name",
			name,
		),

		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_ansible_playbook.example",
			"playbook",
			"mongo_install",
		),

		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_ansible_playbook.example",
			"retry_count",
			"1",
		),

		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_ansible_playbook.example",
			"retry_delay_seconds",
			"10",
		),

		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_ansible_playbook.example",
			"retryable",
			"true",
		),

		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_ansible_playbook.example",
			"skip_tags",
			"web",
		),

		resource.TestCheckResourceAttr(
			"hpe_morpheus_task_ansible_playbook.example",
			"tags",
			"mongo",
		),
	}

	checkFn := resource.ComposeAggregateTestCheckFunc(checks...)
	resource.Test(t, resource.TestCase{
		ProtoV6ProviderFactories: testhelpers.GetAccTestFactories(t, morpheus.New(), sdkv2morpheus.Provider()),
		Steps: []resource.TestStep{
			// Apply
			{
				Config:             providerConfig + resourceConfig,
				ExpectNonEmptyPlan: false,
				Check:              checkFn,
			},
			// Plan after apply
			{
				Config:             providerConfig + resourceConfig,
				ExpectNonEmptyPlan: false,
				PlanOnly:           true,
			},
		},
	})
}
