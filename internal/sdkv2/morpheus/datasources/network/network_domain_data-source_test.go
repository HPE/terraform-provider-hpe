...
func TestAccMorpheusDataSourceNetworkDomainExampleOk(t *testing.T) {
	t.Parallel()
	...
	if currentDependency, err := network.RenderNetworkDomainConfig(t, map[string]string{
		"Name": name,
		"Code": name,
	}); err != nil {
		t.Fatal(err)
	...
	datasourceConfig, err := dsnetwork.RenderNetworkDomainConfig(t, map[string]string{
		"Name": "resource.hpe_morpheus_network_domain.example.name",
	})
	if err != nil {
		t.Fatal(err)
	}
