...
func TestAccMorpheusFindEnvironmentById(t *testing.T) {
	...
	environment := testhelpers.CreateEnvironment(t)
	...
	config := testhelpers.RenderExample(t, "example-id.tf.tmpl", "Id", environmentID)
...
func TestAccMorpheusFindIdbyName(t *testing.T) {
	...
	config := testhelpers.RenderExample(t, "example-name.tf.tmpl", "Name", environmentName)
	checks := []resource.TestCheckFunc{
		resource.TestCheckResourceAttr(
			"data.hpe_morpheus_environment.test",
			"name",
