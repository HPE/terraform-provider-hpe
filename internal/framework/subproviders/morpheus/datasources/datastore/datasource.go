...
func getDatastoreById(
	...
	if err != nil || hresp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("datastore %d GET failed: %s", id, errors.ErrMsg(err, hresp))
	}
	...
	case alletraMPHVMCode:
		...
		keysFromMap, _ := utils.CheckPlanAttributeAgainstAPIAttribute(
...
func populateCloudDatastoreInformation(
	...
	if err != nil {
		retErr := fmt.Errorf("datastore %d cloud datastore GET failed: %s", id, errors.ErrMsg(err, hresp))
...
func populateClusterDatastoreInformation(
	...
	if err != nil {
		retErr := fmt.Errorf("datastore %d cluster datastore GET failed: %s", id, errors.ErrMsg(err, hresp))
...
func getDatastoreByName(
	...
	if err != nil || hresp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("datastore %s list failed: %s", name, errors.ErrMsg(err, hresp))
	}
