...
package clusteraffinitygroupmember_test
...
const resourceName = "hpe_morpheus_cluster_affinity_group_member.test"
...
func checkMemberPresent() resource.TestCheckFunc {
	return func(s *terraform.State) error {
		clusterID, err := stateInt64(s, "cluster_id")
		if err != nil {
			return err
		}
		...
		members := groupMembersNoT(clusterID, groupID)
...
// *testing.T available.
func groupMembersNoT(clusterID, groupID int64) []int64 {
	client, err := testhelpers.NewClientForServer(context.Background(), "")
	if err != nil {
		return nil
	}

	result, _, err := client.ClustersAPI.
		GetClusterAffinityGroup(context.Background(), clusterID, groupID).Execute()
...
func config(name, clusterID, serverID string) string {
	return fmt.Sprintf(`
...
resource "hpe_morpheus_cluster_affinity_group_member" "test" {
  cluster_id        = %[2]s
  affinity_group_id = hpe_morpheus_cluster_affinity_group.test.id
  server_id         = %[3]s
}
`, name, clusterID, serverID)
}

// TestAccMorpheusClusterAffinityGroupMember covers the lifecycle and import.
func TestAccMorpheusClusterAffinityGroupMember(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: providerConfig + config(name, clusterID, serverID),
...
// TestAccMorpheusClusterAffinityGroupMemberCoexists is the reason this resource
// exists.
//
...
func TestAccMorpheusClusterAffinityGroupMemberCoexists(t *testing.T) {
	defer testhelpers.RecordResult(t)
	...
	providerConfig := testhelpers.ProviderBlock()
	name := acctest.RandomWithPrefix(t.Name())
	cfg := providerConfig + config(name, clusterID, serverID)
...
func joinOutOfBand(serverID int64) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		servers := append(groupMembersNoT(clusterID, groupID), serverID)

		body := sdk.UpdateClusterAffinityGroupRequest{
			AffinityGroup: &sdk.UpdateClusterAffinityGroupRequestAffinityGroup{
				Servers: servers,
			},
		}

		if _, _, err := client.ClustersAPI.
			UpdateClusterAffinityGroup(context.Background(), clusterID, groupID).
			UpdateClusterAffinityGroupRequest(body).Execute(); err != nil {
