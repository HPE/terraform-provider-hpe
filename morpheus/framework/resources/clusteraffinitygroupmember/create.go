...
func (r *Resource) Create(
	...
	plan.ID = types.StringValue(membershipID(clusterID, groupID, serverID))
...
func readMembership(
	...
	if !affinityread.IsSingleItemRenderFailure(httpResp) {
		if err := errfmt.CheckResponse(err, httpResp); err != nil {
			errfmt.DiagError(
				diags, errfmt.OpRead, "cluster_affinity_group_member", "", err, httpResp,
			)
	...
	if err := errfmt.CheckResponse(listErr, listResp); err != nil {
		errfmt.DiagError(
			diags, errfmt.OpRead, "cluster_affinity_group_member", "", err, listResp,
		)
...
func writeMembership(
	...
	_, httpResp, err := client.ClustersAPI.
		UpdateClusterAffinityGroup(ctx, clusterID, groupID).
	...
	if err := errfmt.CheckResponse(err, httpResp); err != nil {
		errfmt.DiagError(
			diags, errfmt.OpUpdate, "cluster_affinity_group_member", "", err, httpResp,
		)
