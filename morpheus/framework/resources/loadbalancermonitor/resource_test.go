...
func TestAccMorpheusLoadBalancerMonitorNsxvExampleOk(t *testing.T) {
	defer testhelpers.RecordResult(t)
	...
	lbConfig, err := loadbalancer.RenderLoadBalancerNsxvConfig(t, map[string]string{
		"Name": lbName,
	})
	if err != nil {
		t.Fatalf("failed to render lb config: %s", err)
	...
	resourceName := "hpe_morpheus_load_balancer_monitor.nsxv"

	monitorConfig, err := loadbalancermonitor.RenderLoadBalancerMonitorNsxvConfig(t, map[string]string{
		"LoadBalancerId": "hpe_morpheus_load_balancer.lb.id",
		"Name":           monitorName,
	})
	if err != nil {
	...
	checks := []resource.TestCheckFunc{
		resource.TestCheckResourceAttr(resourceName, "name", monitorName),
		resource.TestCheckResourceAttr(resourceName, "description", "An NSX-V HTTP health check monitor"),
		resource.TestCheckResourceAttr(resourceName, "monitor_type", "http"),
		resource.TestCheckResourceAttr(resourceName, "monitor_interval", "10"),
		resource.TestCheckResourceAttr(resourceName, "monitor_timeout", "15"),
		resource.TestCheckResourceAttr(resourceName, "max_retry", "3"),
		resource.TestCheckResourceAttr(resourceName, "send_type", "GET"),
		resource.TestCheckResourceAttr(resourceName, "receive_code", "200"),
		resource.TestCheckResourceAttr(resourceName, "monitor_destination", "/health"),
		resource.TestCheckResourceAttrSet(resourceName, "id"),
		resource.TestCheckResourceAttrSet(resourceName, "load_balancer_id"),
	}
...
func TestAccMorpheusLoadBalancerMonitorNsxvUpdateOk(t *testing.T) {
	defer testhelpers.RecordResult(t)
	...
	createConfig, err := loadbalancermonitor.RenderLoadBalancerMonitorNsxvConfig(t, map[string]string{
		"LoadBalancerId": "hpe_morpheus_load_balancer.lb.id",
		"Name":           monitorName,
	})
	if err != nil {
	...
	updateConfig, err := loadbalancermonitor.RenderLoadBalancerMonitorNsxvConfig(t, map[string]string{
		"LoadBalancerId":     "hpe_morpheus_load_balancer.lb.id",
		"Name":               updatedName,
		"Description":        "Updated NSX-V monitor",
		"MonitorTimeout":     "30",
		"MaxRetry":           "5",
		"ReceiveCode":        "201",
		"MonitorDestination": "/status",
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(resourceName, "name", updatedName),
					resource.TestCheckResourceAttr(resourceName, "description", "Updated NSX-V monitor"),
					resource.TestCheckResourceAttr(resourceName, "monitor_timeout", "30"),
					resource.TestCheckResourceAttr(resourceName, "max_retry", "5"),
					resource.TestCheckResourceAttr(resourceName, "receive_code", "201"),
					resource.TestCheckResourceAttr(resourceName, "monitor_destination", "/status"),
				),
				PlanOnly: false,
