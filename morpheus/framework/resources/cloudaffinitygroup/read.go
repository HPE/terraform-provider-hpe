...
func (r *cloudAffinityGroupResource) Read(
	ctx context.Context,
	...
	var state CloudAffinityGroupModel
	...
	// Deleting the parent resource pool or cloud cascades and hard-deletes affinity groups.
	// fetchAffinityGroup reports the same for a group absent from the listing.
	ag, ok := fetchAffinityGroup(ctx, client, cloudID, id, &resp.Diagnostics)
	if !ok {
		if resp.Diagnostics.HasError() {
			return
		}
	...
	if isImport {
		resp.Diagnostics.Append(mapGetResponseToModel(ctx, &state, ag, cloudID)...)
	} else {
		resp.Diagnostics.Append(mapAndResolveResponse(ctx, &state, ag, cloudID)...)
	}
...
func mapAndResolveResponse(
	...
	diags := mapGetResponseToModel(ctx, model, ag, cloudID)
...
// resolveGroups resolves resource_permissions.groups, and the Optional+Computed
// `default` inside each element, down to known values.
//
...
func mapGetResponseToModel(
	...
	model.CloudId = types.Int64Value(cloudID)
	...
	// ResourcePermissions — Sites in the response is []map[string]interface{}.
	if ag.ResourcePermissions != nil {
		groupVals := make([]GroupsValue, 0, len(ag.ResourcePermissions.Sites))
		for _, siteMap := range ag.ResourcePermissions.Sites {
			siteID, idOk := extractInt64(siteMap, "id")
			if !idOk {
				continue
			}
			defaultVal := types.BoolNull()
			if dflt, ok := siteMap["default"]; ok {
				if b, bOk := dflt.(bool); bOk {
					defaultVal = types.BoolValue(b)
				}
			}
			groupVals = append(groupVals, GroupsValue{
				Id:      types.Int64Value(siteID),
				Default: defaultVal,
				state:   attr.ValueStateKnown,
			})
		}
...
) (*sdk.GetCloudAffinityGroup200ResponseAffinityGroup, bool) {
...
func affinityGroupFromListEntry(
	e *sdk.ListCloudAffinityGroups200ResponseAllOfAffinityGroupsInner,
) *sdk.GetCloudAffinityGroup200ResponseAffinityGroup {
	ag := &sdk.GetCloudAffinityGroup200ResponseAffinityGroup{
		Id:           e.Id,
		Name:         e.Name,
	...
	if e.Pool != nil {
		ag.Pool = &sdk.GetCloudAffinityGroup200ResponseAffinityGroupPool{Id: e.Pool.Id}
	}

	if e.Servers != nil {
		servers := make([]sdk.GetCloudAffinityGroup200ResponseAffinityGroupServersInner, 0, len(e.Servers))
		for _, s := range e.Servers {
			servers = append(servers, sdk.GetCloudAffinityGroup200ResponseAffinityGroupServersInner{Id: s.Id, Name: s.Name})
		}
