...
// field for regex matching, and whether the field is present. The set of
// supported fields is kept in sync with the OneOf validator on the filter
// block's name attribute (see specs/datasources/storage_volumes/config.yaml).
func storageVolumeFieldValue(
	...
	case "type_name":
		if sv.Type != nil && sv.Type.Name != nil {
			return *sv.Type.Name, true
		}
	case "volume_type":
		if sv.VolumeType != nil {
			return *sv.VolumeType, true
		}
	case "volume_name":
		if sv.VolumeName != nil {
			return *sv.VolumeName, true
		}
	case "volume_path":
		if sv.VolumePath != nil {
			return *sv.VolumePath, true
		}
	case "category":
		if sv.Category != nil {
			return *sv.Category, true
		}
	case "disk_type":
		if sv.DiskType != nil {
			return *sv.DiskType, true
		}
	case "disk_mode":
		if sv.DiskMode != nil {
			return *sv.DiskMode, true
		}
	case "device_name":
		if sv.DeviceName != nil {
			return *sv.DeviceName, true
		}
	case "datastore_id":
	...
	case "storage_group_id":
		if sv.StorageGroup != nil && sv.StorageGroup.Id != nil {
			return strconv.FormatInt(*sv.StorageGroup.Id, 10), true
		}
	case "storage_group_name":
		if sv.StorageGroup != nil && sv.StorageGroup.Name != nil {
			return *sv.StorageGroup.Name, true
		}
	case "status":
		if sv.Status != nil {
	...
	case "source":
		if sv.Source != nil {
			return *sv.Source, true
		}
	case "external_id":
		if sv.ExternalId != nil {
			return *sv.ExternalId, true
		}
	case "internal_id":
		if v := sv.InternalId.Get(); v != nil {
			return *v, true
		}
	case "unique_id":
		if v := sv.UniqueId.Get(); v != nil {
			return *v, true
		}
	case "namespace":
		if v := sv.Namespace.Get(); v != nil {
			return *v, true
		}
	case "storage_profile":
		if v := sv.StorageProfile.Get(); v != nil {
			return *v, true
		}
	}
...
func storageVolumeInnerToValue(
	...
	// type_id comes from the top-level typeId; type_code / type_name come from
	// the nested type object. Mapping type_id from the top-level field (rather
	// than sv.Type.Id) matches the singular data source and the filter logic and
	// keeps type_id populated even when the API omits the nested type object.
