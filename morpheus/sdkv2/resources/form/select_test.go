...
func TestAccMorpheusFormSelectOk(t *testing.T) {
	...
	resourceConfig, err := form.RenderSelectConfig(t, map[string]string{"Name": name})
	if err != nil {
		t.Fatal(err)
	}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("hpe_morpheus_form.example", "option_type1_field_label", "Select Test"),
					resource.TestCheckResourceAttr("hpe_morpheus_form.example", "option_type1_field_name", "selectTest"),
					...
					resource.TestCheckResourceAttr("hpe_morpheus_form.example", "option_type1_name", "tf example select"),
