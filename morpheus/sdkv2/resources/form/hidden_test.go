...
func TestAccMorpheusFormHiddenOk(t *testing.T) {
	...
	resourceConfig, err := form.RenderHiddenConfig(t, map[string]string{"Name": name})
	if err != nil {
		t.Fatal(err)
	}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("hpe_morpheus_form.example", "option_type1_field_label", "hidden input"),
					resource.TestCheckResourceAttr("hpe_morpheus_form.example", "option_type1_field_name", "hiddenInput"),
					...
					resource.TestCheckResourceAttr("hpe_morpheus_form.example", "option_type1_name", "tf hidden input example"),
