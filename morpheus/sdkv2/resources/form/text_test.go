...
func TestAccMorpheusFormTextOk(t *testing.T) {
	...
	resourceConfig, err := form.RenderTextConfig(t, map[string]string{
		"Name":           name,
		"Code":           code,
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("hpe_morpheus_form.example", "option_type.0.type", "text"),
					...
					resource.TestCheckResourceAttr("hpe_morpheus_form.example", "option_type.0.description", "Terraform text example"),
