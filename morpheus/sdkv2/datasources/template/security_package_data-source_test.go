...
func TestAccMorpheusDataSourceSecurityPackageExampleOk(t *testing.T) {
	t.Parallel()
	...
	if currentDependency, err := template.RenderSecurityPackageConfig(t, map[string]string{
		"Name": name,
	...
	datasourceConfig, err := dstemplate.RenderSecurityPackageConfig(t, map[string]string{
		"Name": "resource.hpe_morpheus_security_package.tf_example_security_package.name",
	})
	if err != nil {
		t.Fatal(err)
	}
