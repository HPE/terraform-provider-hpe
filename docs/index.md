...
See the [Legacy provider mapping](https://registry.terraform.io/providers/HPE/hpe/latest/docs/guides/legacy_provider_mapping) guide for the mapping of Morpheus provider resources and data sources to those in this provider.<br><br>
...
release notes for the current version (v2.0.0).

### Authentication

There are four ways to authenticate with Morpheus: a username and password, an access token, PCE
Identity for a Connected PCE (Private Cloud Enterprise) deployment, and PCE Disconnected Identity
for a Disconnected deployment.  See the [Authentication](https://registry.terraform.io/providers/HPE/hpe/latest/docs/guides/morpheus_authentication) guide for the
details of each, with a worked example of every supported provider configuration.
...
OpsRamp resources are configured with an `opsramp` block, authenticating against the OpsRamp API
with OAuth client credentials scoped to a tenant.  Each value may instead be supplied through the
environment.  See the [Authentication](https://registry.terraform.io/providers/HPE/hpe/latest/docs/guides/opsramp_authentication) guide for the details.
...
Release notes for earlier versions are in [HISTORY.md](https://github.com/HPE/terraform-provider-hpe/blob/HEAD/HISTORY.md).
