...
9000 (9060/9080), which share the same plugin and API.

`compute_server_id` and `instance_ids` select where the volume is exported, and
`shared` must match the chosen target:

- `compute_server_id` presents the volume to a **single host** (single-attach),
  so it requires `shared = false`.
- `instance_ids` presents the volume to **every node of each listed instance**
  (multi-attach), so it requires `shared = true`.

`compute_server_id` and `instance_ids` are mutually exclusive, and both pairings
are enforced at plan time. Under the hood the Alletra MP plugin groups an
instance's shared volumes into a single volume set, so a mismatch (for example
`instance_ids` with `shared = false`) would present the volume to multiple hosts
while tracking it as non-shared.
