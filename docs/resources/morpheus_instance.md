...
An instance may have several compute servers — for example, multi-node layouts provision one
compute server per node. These are the values to pass to the `servers` attribute on
...
- `compute_servers` (Set of Number) The IDs of the compute servers backing this instance. An instance may
