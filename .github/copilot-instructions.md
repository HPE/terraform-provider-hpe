...
| File | Purpose |
|---|---|
| `resource.go` | CRUD implementation, `Metadata()`, `Schema()`, API-to-state mapping. When this file exceeds 250 lines, split CRUD operations into separate files: `create.go`, `read.go`, `update.go`, `delete.go`. Keep `resource.go` for `Metadata()`, `Schema()`, shared helpers, and API-to-state mapping. |
